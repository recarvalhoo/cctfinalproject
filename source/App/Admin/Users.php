...
class Users extends Admin
{
    ...
    public function home(?array $data): void
    {
        ...
        if (!empty($data["search"]) && str_search($data["search"]) != "all") {
            ...
            if (!$users->count()) {
                $this->message->info("Sorry, no results were found for your search!")->flash();
    ...
    public function user(?array $data): void
    {
        ...
        if (!empty($data["action"]) && $data["action"] == "create") {
            ...
            $userCreate->datebirth = date_fmt($data["datebirth"]);
            $userCreate->document = preg_replace("/[^0-9]/", "", $data["document"]);
            $userCreate->status = $data["status"];
            $userCreate->role = $data["role"];
            $userCreate->bio = $data["bio"];
            $userCreate->experts = isset($data["experts"]) ? 1 : 0;
            $userCreate->boss = isset($data["boss"]) ? 1 : 0;

            $userCreate->linkedin = $data["linkedin"];
            $userCreate->facebook = $data["facebook"];
            $userCreate->twitter = $data["twitter"];
            $userCreate->instagram = $data["instagram"];
            $userCreate->youtube = $data["youtube"];
            ...
            $this->message->success("User successfully registered!")->flash();
        ...
        if (!empty($data["action"]) && $data["action"] == "update") {
            ...
            if (!$userUpdate) {
                $this->message->error("Sorry, this is an error, try again!")->flash();
            ...
            $userUpdate->datebirth = date_fmt($data["datebirth"]);
            ...
            $userUpdate->experts = isset($data["experts"]) ? 1 : 0;
            $userUpdate->boss = isset($data["boss"]) ? 1 : 0;

            $userUpdate->linkedin = $data["linkedin"];
            $userUpdate->facebook = $data["facebook"];
            $userUpdate->twitter = $data["twitter"];
            $userUpdate->instagram = $data["instagram"];
            $userUpdate->youtube = $data["youtube"];
            ...
            $this->message->success("User successfully updated!")->flash();
        ...
        if (!empty($data["action"]) && $data["action"] == "delete") {
            ...
            if (!$userDelete) {
                $this->message->error("You tried to deleted an user that doesn't exist!")->flash();
                echo json_encode(["redirect" => url("/admin/users/home")]);
                return;
            }
            ...
            $this->message->success("User successfully deleted!")->flash();
            echo json_encode(["redirect" => url("/admin/users/home")]);
        ...
        $head = $this->seo->render(
            CONF_SITE_NAME . " | " . ($userEdit ? "Profile of {$userEdit->fullName()}" : "New user"),
            CONF_SITE_DESC,
            url("/admin"),
            url("/admin/assets/images/image.jpg"),
            false
